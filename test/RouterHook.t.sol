...
contract RouterHookTest is Test, Deployers {
    ...
    function test_router_can_factory_and_mint_tokens() public {
        address pairPool = routerHook.rebalancerFactory(Currency.unwrap(token0), Currency.unwrap(token1));
        ...
        MockERC20(Currency.unwrap(token0)).mint(pairPool, 10 ether);
        MockERC20(Currency.unwrap(token1)).mint(pairPool, 10 ether);

        assertEq(MockERC20(Currency.unwrap(token0)).balanceOf(pairPool), 10 ether);
        assertEq(MockERC20(Currency.unwrap(token1)).balanceOf(pairPool), 10 ether);
    }
}
